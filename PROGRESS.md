...
20. 上线前认证安全收口
   - 登录失败默认按来源地址与规范化用户名计数，15 分钟内达到 5 次后限制 15 分钟，返回 `429` 与 `Retry-After`。
   - 限流计数器为有界进程内存储，默认最多 10000 个键；单实例部署可用，多实例时需换成共享存储。
   - 新增用户自助修改密码与管理员重置其他用户密码接口，持续使用 BCrypt cost 12，并禁止复用当前密码。
   - 密码变更后通过 Spring Security SessionRegistry 使目标用户所有现有会话失效，写入 `PASSWORD_CHANGED` 或 `PASSWORD_RESET` 审计记录。
   - 前端提供顶部“修改密码”入口及管理员用户列表“重置密码”入口，表单校验两次新密码一致。
21. 第二期 / 统一工作项切片 1（基础与状态历史）
   - Flyway V10 新增 `work_item` 和只追加的 `work_item_status_log`，首批支持任务、里程碑、四级优先级与计划/实际日期。
   - 实现创建、组合筛选分页、详情、局部修改、幂等软删除、受控状态流转和状态历史 API。
   - 新增 `work_item:read/write/delete` 权限并叠加项目资源级授权；OWNER、MANAGER 和管理员可完整维护，被指派成员可编辑内容与流转状态，VIEWER 保持只读。
   - 创建、修改、删除和状态变更均写入审计；状态变更使用乐观条件防止并发覆盖。
22. 第二期 / 统一工作项切片 2（关系）
   - Flyway V11 新增有向工作项关系，支持父子、前置和阻塞三种类型，并用复合外键限定关系两端属于同一项目。
   - 实现关系创建、按工作项/类型分页筛选和幂等软删除 API，响应直接返回两端工作项摘要。
   - 禁止自关联、跨项目、同方向重复和循环关系；父子图独立判环，前置与阻塞共享依赖图判环。
   - 关系写入通过项目行锁串行化判重与判环，由唯一索引提供最终并发保护；创建和删除均写入审计。
   - 项目成员可读，仅 OWNER、MANAGER 和系统管理员可维护关系，同时继续检查 `work_item:read/write` 功能权限。
23. 第二期 / 统一工作项切片 3（前端工作项页面）
   - 项目工作台新增权限感知的“工作项”模块，支持关键词、类型、状态、优先级和负责人筛选。
   - 实现工作项创建、详情、编辑、删除、计划/实际周期展示，并为输入错误、空数据、加载与请求失败提供明确状态。
   - 实现状态机目标选择、变更备注和时间线历史，流转后同步刷新列表、详情与历史。
   - 实现父子、前置和阻塞关系的方向选择、创建、展示和移除，创建前明确提示箭头语义。
   - 前端同时参考 `work_item:*` 功能权限、项目 OWNER/MANAGER 角色、管理员身份和当前负责人隐藏不可用操作，但不取代服务端授权。
24. 第二期 / 统一工作项切片 4（看板、日历与个人提醒）
   - 工作项页面新增列表、看板、日历三种视图；看板按四种状态分栏，卡片可打开详情，具备权限的用户可直接发起状态流转。
   - 月历按计划周期查询并铺展工作项，支持前后月份切换；仅设置计划开始或计划结束日期的工作项也按单日计划展示。
...
- 第二期 / 统一工作项切片 4，2026-09-24：
  ...
  - 后端提醒专项 3 passed；执行后端全量 `mvn clean test`：93 passed，0 failed，0 errors，0 skipped。测试 JVM 退出阶段仍有 Testcontainers 已关闭连接的 Hikari 清理告警，但不影响测试结果。当前未使用真实账号执行浏览器写操作。
  - 生产构建仍有现有主包大于 500 kB 的性能警告，不影响功能；后续可按项目模块做懒加载拆包。

- 第二期 / 统一工作项切片 3，2026-09-24：
  - 按 RED → GREEN 新增 `work-items.spec.ts` 和 `WorkItemPanel.spec.ts`，并扩展 `ProjectWorkspace.spec.ts`；覆盖 API 路径/筛选序列化、列表与详情加载、状态历史、关系、只读权限隐藏和项目模块接入。
  - 执行前端全量 `pnpm test`：23 个测试文件、61 passed；执行 `pnpm build`：TypeScript 检查与 Vite 生产构建成功。
  - 真实浏览器确认本地应用可正常加载到登录页；由于未获授权使用或重置开发账号密码，未在真实会话中执行工作项写操作。
  - 生产构建仍有现有主包大于 500 kB 的性能警告，不影响本切片功能，后续可配合路由/模块懒加载一并优化。

- 第二期 / 统一工作项切片 2，2026-09-24：
  - 按 RED → GREEN 新增 `WorkItemRelationApiTest`；覆盖创建、筛选、幂等删除、审计、自关联/跨项目/重复拒绝、父子图循环、混合依赖图循环、功能权限、项目角色与 CSRF，共 4 passed。
  - 更新空库迁移基线断言；执行后端全量 `clean package`：90 passed，0 failed，0 errors，0 skipped，JAR 打包成功。
  - 本切片仅修改后端与文档，未改动前端，因此未重复执行前端测试。

- 第二期 / 统一工作项切片 1，2026-09-24：
  - 按 RED → GREEN 新增 `WorkItemApiTest`；覆盖创建、筛选、详情、PATCH 清空字段、状态机、实际日期、历史、幂等删除、审计、功能权限、项目角色、指派成员、VIEWER 保护、CSRF 与输入校验，共 3 passed。
  - 更新空库迁移和 RBAC 矩阵断言；执行后端全量 `clean package`：86 passed，0 failed，0 errors，0 skipped，JAR 打包成功。
  - 本切片仅提供后端基础 API；关系、前端页面、看板、日历和提醒仍按已确认的后续切片实现。

- 上线前认证安全收口，2026-09-24：
  - 按 RED → GREEN 新增登录限流和密码管理集成测试；覆盖失败阈值、用户名隔离、旧密码校验、密码复用禁止、管理员不可重置自己、审计与全会话失效。
  - 执行后端全量 `clean package`：83 passed，0 failed，0 errors，0 skipped，JAR 打包成功。首次全量运行曾因 Testcontainers PostgreSQL 连接瞬断出现 2 个环境错误，迁移测试单独重跑及全量再跑均通过。
  - 执行前端 `pnpm typecheck && pnpm test`：21 个测试文件共 55 passed；`pnpm build` 生产构建成功。
  - 执行 `pnpm audit --prod --registry=https://registry.npmjs.org`：未发现已知生产依赖漏洞。默认 npmmirror 不提供 audit 端点，因此显式使用 npmjs 官方端点复验。
...
- 登录限流和会话登记当前是单实例进程内状态；如果后续水平扩容后端，必须切换为 Valkey 等共享限流存储与共享 Session 存储。
